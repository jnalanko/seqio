...
public:
    ...
    void flush(){
        throw std::runtime_error("Error: zstr::ofstream flush() does not actully flush, so you should not call it. Manually flushing a zlib stream mid-stream is apparently bad practice and leads to bad performance. Instead, you should let the object go out of scope, which will close the stream, which will flush it.");
    }
